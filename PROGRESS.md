...
- Frontend Playwright `tests/threat-lab.spec.ts` — **19 tests** on the live backend (payload captured from the same `POST /embodiment/run` and compared with the DOM): load, pre-run no fake activity, RUN replays, object approaches, looming / eye / brain follow the record, ESCAPE marker + motor + story highlight, fly moves only per `BodyState`, PAUSE / PLAY, STEP ▶ ◀, slider sync of every panel, RESET, failure → NO RESULT (route-mocked 500), azimuth preset → recorded direction, out-of-range params rejected client-side and 422 server-side, determinism, hero / tab / hash routing, P5 demo still works, P6 inspector still works, biological identity vs simulated activity. `tests/smoke-threat-lab.spec.ts` — 5 screenshot tests. Full Playwright suite: **83 passed** (59 + 24); typecheck + production build clean.
